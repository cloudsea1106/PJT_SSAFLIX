...
기획부터 코드 작성까지 쉴 새 없이 달린 일주일이었습니다. 장고를 오래 배웠다고 생각했음에도 불구하고 데이터를 저장하고 불러오는 과정에서 생각지 못한 오류가 많이 발생했습니다. 오류를 해결하기 위해 postman으로 코드 한 줄 한 줄 확인해가며 디버깅한 일이 기억에 남습니다. 간단한 코드라도 당연히 될 것이라 생각하지 말고 반드시 확인해가며 작성해야함을 느꼈습니다.

fixtures를 저장하기 위해 dumpdata를 했는데, 저장 데이터에 한글이 작성되어 오류가 발생했을 때는 수업에서 다루지 않았던 내용이라 당황하기도 했습니다.  다행히 파이썬 공식문서를 통해 UTF-8 인코딩을 사용하여 해결할 수 있었습니다. 이 외에도 데이터를 불러오며 쿼리를 작성할 때 어려움을 겪었지만 장고 공식 문서를 통해 다양한 쿼리 작성법을 배울 수 있었고 원하는 데이터를 추출할 수 있었습니다. 문제가 발생해도 스스로 해결하고자 공식 문서를 찾고 시도해보며 마침내 해결했을 때 뿌듯함을 느꼈습니다.

혼자 하는 것이 아닌 협업이기 때문에 어떻게 하면 뷰에서 데이터를 더 깔끔하게 받을 수 있을까 고민하며 코드를 작성했습니다. 일주일 동안 더 좋은 코드를 고민하며 함께해 주신 태삼님께 감사드리며, 저 또한 서로 도움주는 팀원이었길 바랍니다.
...
# 3-2. Vue - 김태삼 
 ...
 실습보다 훨씬 복잡한 프로젝트를 진행하다보니 router, view, components등 고려해야할 부분이 많고, 전체적인 구조를 파악하고 있는게 가장 어려웠었다.  구조적인 측면에서는 view에 있는 vue파일과 components의 vue파일들을 명확하게 구분해놓지 못한 것이 아쉬웠다. 이를 통해 프로젝트 시작단계에서 전체적인 템플릿과 세부적인 템플릿을 구조화하는 과정의 중요성을 느꼈다.
